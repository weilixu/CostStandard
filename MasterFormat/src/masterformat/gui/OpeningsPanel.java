...
public class OpeningsPanel extends JPanel implements TreeSelectionListener {
    ...
    public OpeningsPanel(EnergyPlusModel m, String open) {
	...
	if (node.isLeaf()) {
	    ...
	    try {
		model.setTransparentMaterialMasterFormat(openName, tn.getDescription());
	    } catch (Exception e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	    }
	    userInputs = model.getOpeningsUserInputs(openName);
	    model.getOpeningsCostVector(openName);
	...
	model.getOpeningsOptionList(openName);
	model.getOpeningsOptionQuantities(openName);
