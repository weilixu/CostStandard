...
public class PSZAC implements HVACSystem{
    //recording all the required data for PSZAC
    ...
    public PSZAC(HashMap<String, ArrayList<EplusObject>>objects, EnergyPlusBuildingForHVACSystems bldg){
	objectLists = objects;
	buildings = bldg;
	...
	while(zoneMapIterator.hasNext()){
		...
		roomCounter ++;
		demandSideSystem.addAll(processSupplyTemp(zone.getFullName(), demandSideSystemTemplate));
		demandSideSystem.add(zone.getOutdoorAirObject());
		supplySideSystem.addAll(processDemandTemp(zone.getFullName(), supplySideSystemTemplate));
	    }
	}
