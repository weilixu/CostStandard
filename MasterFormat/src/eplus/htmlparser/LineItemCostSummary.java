...
public class LineItemCostSummary {
	...
	doc = d;
	lineItemCostSummaryTable = doc.getElementsByAttributeValue(TAG, LINE_ITEM_COST_TABLE_ID);
	lineItemList = new ArrayList<LineItem>();
	processData();
    }
    
    private void processData(){
	Elements costItemList = lineItemCostSummaryTable.get(0).getElementsByTag("td");
	for(int i=0; i<costItemList.size(); i++){
	    if(costItemList.get(i).text().equals("--") && (i+subTotal)<costItemList.size()){
