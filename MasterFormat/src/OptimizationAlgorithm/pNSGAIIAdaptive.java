...
public class pNSGAIIAdaptive extends Algorithm {
	...
	while (evaluations < maxEvaluations) {
	    ...
	    if (newGenCounter == 0) {
		for (int i = 0; i < populationSize; i++) {
		    parallelEvaluator_.addSolutionForEvaluation(population
			    .get(i));
		}
	    } else {
...
//	    population
//		    .printFeasibleFUN("E:\\02_Weili\\02_ResearchTopic\\PhD Case Study\\OneMP\\ParetoFronts\\Front"
//			    + generation);
//	    population.printFeasibleVAR("E:\\02_Weili\\02_ResearchTopic\\PhD Case Study\\OneMP\\ParetoFronts\\Var" +
//			    generation);
    ...
    // check one solution is sufficient for this problem
    private boolean checkDuplicates(SolutionSet pop, Solution s) {
	for (int i = 0; i < pop.size(); i++) {
	    Double answer = pop.get(i).getObjective(0);
	    if (answer == s.getObjective(0)) {
		return true;
	    }
	}
	return false;
